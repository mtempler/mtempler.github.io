...
When applied to a "need", say, electrical power, **Scientific Learning** becomes the **Science of Giving and Receiving**.  With both the supply and demand time-series, the power grid analytically realizes the Science of Giving and Receiving.  The science of giving and receiving is used to re-architect the electrical power grid for non-equilibrium operation to support variable energy sources (renewables).  The power grid of the future is a re-engineering problem that is not solved by government policy alone.
