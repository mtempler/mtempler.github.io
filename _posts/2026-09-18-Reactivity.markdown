...
image: img-04.jpg
---
In a previous blog post we examined the science-of-counting in units of measure, and applied it to the design of non-equilibrium power grids.  We saw that the interaction in scientific learning between two time-series, source and load, is solvable and given by a planar cubic curve.  Our focus on the power grid continues because operational risk, while still significant, is reduced since the conventional grid is built on a simplified science-of-counting (where average strain vanishes, at statistics).  The engineering assumption was valid when the generated power was known and stable, however, renewables make the assumption invalid.  Our objective is to clarify the cooperation between power source and power load to regulate the energy, and introduce scientific instrumentation to improve fault detection and self-healing of the network.
...
With strain zero-ed out, the 19th and 20th centuries strayed from the purely deductive path, and human invention took over to make the system operational.  It was discovered that AC voltage regulation required the re-introduction of strain (energy storage), expressed as imaginary values for power (reactivity) due to the introduction of reactive devices, capacitors and inductors.  In this blog post we look at how reactivity emerges naturally from the full science-of-counting with strain, without turning to AC analyses, and how cooperation between power source and power load can lead to successful energy regulation, and instrumentation for fault detection and self-healing. 
