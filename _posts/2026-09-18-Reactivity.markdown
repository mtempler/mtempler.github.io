...
image: img-04.jpg
---
In a previous blog post we examined the science-of-counting in units of measure, and began to apply it to the design of a non-equilibrium power grid.  The focus continues on the electrical power grid, because operational risk is already reduced since the conventional grid is built on a simplified science-of-counting (where average strain vanishes, at statistics).  The engineering assumption is valid when the generated power is known and stable, however, renewables make the assumption invalid.  Our objective is to clarify the cooperation between power source and power load to successfully regulate the energy for a reliable and sustainable supply of electrical power, and scientific instrumentation to improve fault detection and self-healing of the network.
