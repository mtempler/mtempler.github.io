...
image: img-04.jpg
---
In a previous blog post we examined the science-of-counting in units of measure, and applied it to the design of a non-equilibrium power grid.  We saw that the interaction in scientific learning between two time-series, source and load, is solvable and given by a planar cubic curve.  Our focus is on the power grid, because operational risk, while still significant, is reduced since the conventional grid is built on a simplified science-of-counting (where average strain vanishes, at statistics).  The engineering assumption was valid when the generated power is known and stable, however, renewables make the assumption invalid.  Our objective is to clarify the cooperation between power source and power load to regulate the energy, and introduce scientific instrumentation to improve fault detection and self-healing of the network.
